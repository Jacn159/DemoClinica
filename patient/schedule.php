...
// Obtener los scheduleid de las citas del paciente
$sqlScheduleIds = "SELECT scheduleid FROM appointment WHERE pid = ?";
$stmtScheduleIds = $database->prepare($sqlScheduleIds);
$stmtScheduleIds->bind_param("i", $userid); // Supongo que $userid es el ID del paciente
$stmtScheduleIds->execute();
$resultScheduleIds = $stmtScheduleIds->get_result();

if ($resultScheduleIds) {
    // Crear un arreglo para almacenar los scheduleid
    $scheduleIds = array();
    while ($row = $resultScheduleIds->fetch_assoc()) {
        $scheduleIds[] = $row['scheduleid'];
        
    }

    // Ahora, puedes comparar cada scheduleid con $scheduleid
    foreach ($scheduleIds as $scheduleId) {
        // print($schedule_id);
        if ($schedule_id == $scheduleId) {
            // $rowAppointments = $resultAppointments->fetch_assoc();
            // $userHasAppointment = $rowAppointments["count"] > 0;
            echo '
            <td style="width: 25%;">
                <div class="dashboard-items search-items">
                    <div style="width:100%">
                        ...
                        <br>
                        <a href="booking.php?id='.$schedule_id.'" ><button class="login-btn btn-primary-soft btn " style="padding-top:11px;padding-bottom:11px;width:100%"><font class="tn-in-text">Book Now</font></button></a>
                    </div>
                </div>
            </td>';
        }
